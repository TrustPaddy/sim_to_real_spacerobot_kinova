%% deploy_agent_kinova_robust_timing2.m  (V2.3 - Trajektorie + korrigierte Observation; intern "traj_fk")
...
%
% Basis: deploy_agent_kinova_robust_timing.m (Halbkreis-Trajektorie,
% robuste Zeitbasis, Timing-Diagnose, FK-vs-Kortex-Check).
%
% AENDERUNG gegenueber robust_timing (V2.2):
%   - Observation-Konstruktion jetzt IDENTISCH zu deploy_agent_kinova_p2p_10hz:
%       * Reihenfolge (29D): [ep; ev; v_base; w_base; q; dq; e_ori]
%         (robust_timing hatte [ep; ev; q; dq; v_base; w_base; e_ori]).
%         obsLow/obsHigh sind in derselben Reihenfolge mitgezogen.
%       * Fehler-Vorzeichen = Trainings-Konvention (state_minus_target):
%           ep = ee_ist - ee_soll   (vorher: ref - ee)
%           ev = vee   - vref       (vorher: ref - ee)
%       * e_ori fest [0;0;0] (Basis-Orientierungsfehler, fester Kinova).
%         Vorher wurde faelschlich die EE-Orientierung aus ee_rot in e_ori
%         eingespeist -> Phantom-Base-Tilt. Das ist jetzt behoben.
%   - Die EE-Referenztrajektorie (Halbkreis in xz, referenceTrajectory)
%     bleibt aus robust_timing unveraendert.
%
% AENDERUNGEN aus robust_timing (V2.2) gegenueber V2.1:
...
cfg.seed = 11;   % manuell pro Lauf hochzaehlen: 1, 2, 3, ...
...
if strlength(cfg.agentFile) == 0
    d = dir(sk_path("SavedAgents/MotionProfile/Circle/PPO/ppo_10hz.mat"));
    if isempty(d)
...
%  =========================
% Reihenfolge MUSS zur obs-Konstruktion im Loop passen (== p2p_10hz):
...
for k = 1:nStepsMax
    ...
    % --- Fehler-Konvention IDENTISCH zu point_fk / Training ---
    % Trainings-Konvention (bestaetigt): ee_diff = ee_ist - ee_soll.
    % (Frueher hier: ref - ee -> falsches Vorzeichen ggue. Training.)
    ee_vel  = J_ee(4:6, :) * dq;
    ep      = ee_pos(:) - ref_pos;
    ev      = ee_vel(:) - ref_vel;

    % --- Basis-Groessen v_base, w_base, e_ori: fester Kinova -> alle 0 ---
    % e_ori ist eine BASIS-Groesse (Orientierungsfehler des frei
    % schwebenden SpaceKinova-Bodies). Auf dem fest montierten realen
    % Kinova ist die Basis-Orientierung konstant -> e_ori IMMER [0;0;0].
    % NICHT mehr aus ee_rot berechnen: das fuetterte faelschlich die
    % EE-Orientierung in e_ori ein und erzeugte den Phantom-Base-Tilt.
    v_base = zeros(3, 1);
    w_base = zeros(3, 1);
    e_ori  = zeros(3, 1);

    % --- Observation (29D) ---
    % Reihenfolge: [ep; ev; v_base; w_base; q; dq; e_ori]  (== p2p_10hz)
    ...
    % --- Uniform Speed-Scale ---
    dq_cmd = 0.35 * cfg.speedScale * dq_cmd;
