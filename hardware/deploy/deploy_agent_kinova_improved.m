%% deploy_agent_kinova.m  (V2.1 - mit erweitertem Logging fuer C3)
...
%   - Multi-Step-Dump (k=1,2,3,5,10,20,50,100) NACH der Pipeline, mit
%     dq_raw, yFilt, dq_cmd, dq_meas - zeigt Divergenz-Verlauf live
%   - log.dq_filt und log.ep_norm zusaetzlich gespeichert
%   - run_logger-Integration am Ende (C3-kompatibles data/meta-Schema)
%   - Eskalations-Warnung bei ||ep|| > 0.2 m vor dem OOD-Stopp
%
...
cfg.center = [0.497, -0.005, 0.936 + cfg.r];
...
if strlength(cfg.agentFile) == 0
    d = dir(sk_path("SavedAgents/MotionProfile/NonSingular/ppo.mat"));
    if isempty(d)
...
%_ref    = cfg.center(3) + cfg.r * cos(cfg.omega * t_ref);
...
for k = 1:nSteps
    ...
    % --- Referenz + Fehler ---
    tRef = min(tNow, cfg.maxDuration);
    % ref_pos = [ ...
    %     cfg.center(1) + cfg.r * sin(cfg.omega * tRef); ...
    %     cfg.center(2) + cfg.yConst * tRef; ...
    %     cfg.center(3) + cfg.r * cos(cfg.omega * tRef) ];
    % 
    % ref_vel = [ ...
    %     cfg.r * cfg.omega * cos(cfg.omega * tRef); ...
    %     cfg.yConst; ...
    %    -cfg.r * cfg.omega * sin(cfg.omega * tRef) ];
    ref_pos = [ ...
        cfg.center(1) + cfg.r * sin(cfg.omega * tRef); ...
        cfg.center(2) + cfg.yConst * tRef; ...
        cfg.center(3) - cfg.r * cos(cfg.omega * tRef) ];
    ...
    % Zeigt komplette Wirkungskette: was Agent will (dq_raw) -> was Filter
    % macht (yFilt) -> was an Robot geht (dq_cmd) -> was Robot tatsaechlich
    % macht (dq_meas).
...
% Robuste Bestimmung von k_end: nimm den letzten Step bei dem ee_ref
% tatsaechlich gesetzt wurde (nicht mehr 0,0,0 vom initLog)
% k_end kommt direkt aus der Schleife — keine Nachkorrektur nötig.

if k_end < 1
    k_end = 1;
end

% fprintf('[DEBUG] k_end aus Loop:    %d\n', k_end);
% fprintf('[DEBUG] k_end_real (heur): %d\n', k_end_real);
% fprintf('[DEBUG] last nonzero ee_pos: %d\n', find(any(log.ee_pos ~= 0, 2), 1, 'last'));
% fprintf('[DEBUG] last nonzero ee_ref: %d\n', find(any(log.ee_ref ~= 0, 2), 1, 'last'));
% fprintf('[DEBUG] last nonzero log.t : %d\n', find(log.t ~= 0, 1, 'last'));
...
logFile = sk_path("data", "hardware", "deploy_logs", sprintf("deploy_log_%s.mat", ...
...
yline([-1 1], 'r--');
xlabel('t [s]'); ylabel('dq_{raw}'); title('Agent-Output (Pipeline-Input, rad/s)');
legend(arrayfun(@(j) sprintf("J%d", j), 1:cfg.nJ, 'UniformOutput', false));
grid on;
subplot(3,1,2);
plot(log.t, log.yFilt);
xlabel('t [s]'); ylabel('y_{filt} [rad/s]');
...
plot(log.ee_pos(:,1),    log.ee_pos(:,3),    'b--', 'LineWidth', 1.3); hold on;
if ~any(isnan(log.ee_kortex(:)))
...
if ~any(isnan(log.ee_kortex(:)))
    legend('FK-Prediction (Agent sieht)', 'Kortex-Wahrheit (echter Robot)', ...
           'Soll (Referenz)', 'Start FK', 'Start Kortex', 'Start Soll', ...
           'Location', 'best');
else
    legend('FK-Prediction', 'Soll (Referenz)', 'Start FK', 'Start Soll', ...
           'Location', 'best');
end
...
if numel(log.t) > 1
    ...
    if max(dt_real) > 1.5 * cfg.Ts
        warning("Loop verpasst 40 Hz deutlich. Referenz muss zwingend wall-clock-basiert sein.");
    end
end
