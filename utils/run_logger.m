...
%   durchnummeriert und mit Metadaten versehen in data/hardware/runs/ landet.
%
%   USAGE:
%       filepath = run_logger(data, meta)
%       filepath = run_logger(data, meta, 'runDir', sk_path('data', 'hardware', 'runs'))
...
%       fp = run_logger(data, meta);
    ...
    p = inputParser;
    addParameter(p, 'runDir', sk_path('data', 'hardware', 'runs'), @(x) ischar(x) || isstring(x));
