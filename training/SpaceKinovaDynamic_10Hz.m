%% SpaceKinovaDynamic_10Hz.m
% Konfiguration des 10-Hz-Agenten aus dem Paper (ppo_10hz.mat): singulaere Halbkreisbahn, T = 8.5 s,
% 10 Hz, Hyperparameter wie in SavedAgents/MotionProfile/Circle/PPO/ppo_10hz.mat gespeichert.
...
% ---- Dateien/Modelle ----
cfg.urdfFile   = sk_path("robot", "SpaceKinova.urdf");   % <- deine generierte URDF
...
% % t=0   : Startpunkt  (oben)
% P1 = [cfg.center(1), cfg.center(2), cfg.center(3) + cfg.r];
% % t=T/2 : Mittelpunkt  (rechts, Apex)
% P2 = [cfg.center(1) + cfg.r, cfg.center(2), cfg.center(3)];
% % t=T   : Endpunkt    (unten)
% P3 = [cfg.center(1), cfg.center(2), cfg.center(3) - cfg.r];
% 
...
% Optional: feste Achsengrenzen, damit die Ansicht nicht springt
xlim(ax, [-1 1]);
ylim(ax, [-1 1]);
zlim(ax, [-1 2]);
...
for k = 1:10:size(q_des,1)
    show(robot_rbt, q_des(k,:), 'PreservePlot', false, 'Frames', 'off');
    view(ax, 15, 20);
    hold on;
...
% agent.AgentOptions.ActorOptimizerOptions.LearnRate  = 5e-4;
