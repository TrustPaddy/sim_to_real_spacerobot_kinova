...
%
% Voraussetzungen:
%   - setup_project.m wurde ausgefuehrt
%   - Trainierter Agent als .mat-Datei (Variable 'agent')
%   - Simulink-Modell aus models/ (siehe mdl). Zeiten unten passend setzen:
%     40 Hz: Ts = 0.005, Ts_agent = 0.025 | 10 Hz: Ts = 0.02, Ts_agent = 0.1
...
N_episodes = 50;   % Paper: 50 Evaluations-Episoden

% --- Simulink-Modell ---
mdl = 'SpaceKinova_MotionProfile_40Hz';   % 10 Hz: 'SpaceKinova_MotionProfile'

% --- URDF ---
urdfFile = sk_path('robot', 'SpaceKinova.urdf');
...
%agentFile = sk_path('SavedAgents/MotionProfile/CDR/PPO/Trajectory2.mat');
agentFile = sk_path('SavedAgents/MotionProfile/Circle/PPO/SpaceKinova_PPO_agent_motionprofile.mat');
...
for i = 1:N_episodes
    ...
    % % --- Sim-Episode im run_logger-Schema speichern (sim2real) ---
    % lg     = simOut.logsout;
    % EE_ts  = lg.getElement('p_EE').Values;
    % t_ep   = EE_ts.Time(:);
    % ee_ep  = reshape_time_series(EE_ts.Data);                       % N x 3 [m]
    % ref_ep = interp1(EE_ref.Time, EE_ref.Data, t_ep, 'linear', 'extrap');
    % ep_ep  = ee_ep - ref_ep;
    % 
    % dqc_ts = lg.getElement('dq_cmd').Values;
    % dqc_ep = reshape_time_series(dqc_ts.Data);                      % N x 7 [rad/s]
    % dqc_t  = dqc_ts.Time(:);
    % if numel(dqc_t) ~= numel(t_ep)
    %     dqc_ep = interp1(dqc_t, dqc_ep, t_ep, 'linear', 'extrap');
    % end
    % 
    % data = struct();
    % data.t           = t_ep;
    % data.ee_measured = ee_ep;             % [m], Modell-EE (p_EE)
    % data.ee_ref      = ref_ep;            % [m]
    % data.ep          = ep_ep;             % [m]
    % data.ep_norm     = vecnorm(ep_ep, 2, 2);
    % data.dq_cmd      = rad2deg(dqc_ep);   % [deg/s] -> spiegelt reales Schema 1:1
    % 
    % meta = struct();
    % meta.label       = sprintf('sim_circle_seed%d', i);
    % meta.source      = 'sim';
    % meta.seed        = i;
    % meta.rateHz      = 1/Ts_agent;
    % meta.maxDuration = T;                 % WICHTIG: hier 8.5 vs. real 17 -> s.o.
    % meta.units       = struct('ee','m','dq_cmd','deg');
    % meta.comment     = 'Sim-Episode (KPI-Eval) im run_logger-Schema';
    % 
    % % --- Sim-Run direkt nach Deploy_Scripts/runs/ schreiben ---
    % runDir = sk_path('data', 'hardware', 'runs');
    % saveRun(data, meta, runDir);
...
% dq_cmd speichern (Eingabe fuer die Playback-Skripte). Nicht jedes Modell loggt das Signal.
dq_el = logsouts{1}.getElement('dq_cmd');
if isempty(dq_el)
    ...
    save(sk_path('data', 'dq_cmd', 'dq_cmd_non-singular_10hz.mat'), 't', 'dq');
end
...
function fpath = saveRun(data, meta, runDir)
% Speichert einen Run im run_logger-kompatiblen Schema (data + meta) als
% run_NNN_<label>.mat. NNN wird automatisch hochgezaehlt, damit bestehende
% (reale) Runs im selben Ordner NICHT ueberschrieben werden.
if ~exist(runDir, 'dir'); mkdir(runDir); end

existing = dir(fullfile(runDir, 'run_*.mat'));
nextIdx  = 1;
for k = 1:numel(existing)
    tok = regexp(existing(k).name, '^run_(\d+)_', 'tokens', 'once');
    if ~isempty(tok)
        nextIdx = max(nextIdx, str2double(tok{1}) + 1);
    end
end

fname = sprintf('run_%03d_%s.mat', nextIdx, char(meta.label));
fpath = fullfile(runDir, fname);
save(fpath, 'data', 'meta');
fprintf('  Sim-Run gespeichert: %s\n', fpath);
end